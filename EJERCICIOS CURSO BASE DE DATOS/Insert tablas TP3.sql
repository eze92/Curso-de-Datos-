...
DELIMITER $$
CREATE FUNCTION get_name_proveedor (p_idProvedor varchar(10) )
RETURNS VARCHAR(255)
...
select idProveedor, get_name_proveedor(idProveedor) from proveedor;



DELIMITER $$
CREATE FUNCTION get_factura (p_fecha datetime )
RETURNS VARCHAR(255)
...
select f.idNumeroFact as "Numero Factura", f.estado, f.idPedido as Pedido,get_factura(CURDATE() ) as "Fecha Factura"
from factura f ;
...
DELIMITER $$
CREATE FUNCTION get_precioPorMoneda (precio decimal(20,5), simbolo_moneda varchar(5))
RETURNS VARCHAR(255)
...
BEGIN   
    CASE 
        WHEN (precio BETWEEN 15.0 AND 20.0 ) AND simbolo_moneda = 'U$S' THEN RETURN 'MONTO BAJO';
        WHEN (precio >= 21.0 AND  precio <= 40.0 AND simbolo_moneda = 'U$S' )  THEN RETURN 'MONTO MEDIO';
        WHEN (precio >= 41.0  AND simbolo_moneda = 'U$S' ) THEN RETURN 'MONTO ALTO';
        
        WHEN (precio BETWEEN 10000.0 AND 50000.0) AND simbolo_moneda = '$' THEN RETURN 'MONTO BAJO';
        WHEN (precio >= 51000.0 AND  precio <= 71000.0  AND simbolo_moneda = '$' )  THEN RETURN 'MONTO MEDIO';
        WHEN (precio > 71000.0  AND simbolo_moneda = '$' ) THEN RETURN 'MONTO ALTO';
        ELSE RETURN 'MONTO BASICO';
    END CASE;
END
$$

select * ,  get_precioPorMoneda ( precio, simbolo_moneda) from listaPrecio
...
CREATE FUNCTION obtener_campo_fecha(fecha_parametro DATE)
RETURNS DATE
BEGIN
    DECLARE campo_fecha DATE;
    
    SELECT MAX(campo_fecha) INTO campo_fecha
    FROM tu_tabla
    WHERE fecha_columna = fecha_parametro
    AND fecha_actual = CURDATE(); -- O CURDATE() - INTERVAL 1 DAY si quieres la fecha de ayer
...
select * from proveedor where idProveedor = 'P0353' 
...
select * from proveedor where idProveedor in ( 'P0370'  ,'P0356') 
