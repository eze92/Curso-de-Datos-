...
/*drop  DATABASE IF EXISTS centro_diagnostico_investigacion;*/
...
CREATE TABLE IF NOT EXISTS ciudad(
    idCiudad VARCHAR(20) NOT NULL PRIMARY KEY ,
    nombre VARCHAR (100) NOT NULL
);
...
CREATE TABLE IF NOT EXISTS listaPrecio(
	idLista INT NOT NULL PRIMARY KEY NOT NULL ,
    fecha_lista date,
...
CREATE TABLE IF NOT EXISTS pedido(
 ...
 /*tabla para trigger historico*/
CREATE TABLE IF NOT EXISTS proveedor_historico (
...
/*select * from proveedor_historico*/
...
INSERT INTO ciudad(idCiudad,nombre) VALUES ('0000006380','Luque');
INSERT INTO ciudad(idCiudad,nombre) VALUES ('0000007000','Rio De Janeiro');
...
INSERT INTO proveedor (idProveedor, nombre, razon_social, telefono, email, direccion, idPais, idCiudad, idProvincia) VALUES ('P0019', 'AKRIBIS SRL', 'AKRIBIS SRL', '4633-9550', 'info@akribis.info', 'BACACAY 2180 1°B', '600', '0000006380', '09');
...
INSERT INTO proveedor (idProveedor, nombre, razon_social, telefono, email, direccion, idPais, idCiudad, idProvincia) VALUES ('P0065', 'MES LTD', 'MES LTD', '(972) 4 6373981', 'mes@mes-ltd.com', '20 Alon Hatavor Street P.O. Box 3017 Caesarea Industrial Park 3088900', '858', '0000022006', '03');
...
INSERT INTO proveedor (idProveedor, nombre, razon_social, telefono, email, direccion, idPais, idCiudad, idProvincia) VALUES ('P0068', 'AIR CLASS CARGO SA', 'AIR CLASS CARGO SA', '5252-9030', 'ADMINISTRACION@AIRCLASSCARGO.COM', 'SARMIENTO 1190', '076', '0000007000', '012');
...
/*vistas*/
create view view_informacion_proveedores as
SELECT 
p.nombre as "Nombre Proveedor", p.razon_social,p.telefono,p.email, p.direccion, pa.nombre as "Pais", pa.desc_idioma as "Idioma", 
pro.nombre as "Provincia", c.nombre AS "Ciudad"
FROM proveedor p
inner join provincia pro on pro.idProvincia = p.idProvincia
inner join pais pa on pa.idPais =  pro.idPais
inner join ciudad c on c.idCiudad = p.idCiudad;

create view view_informacion_facturas_2024 as
select 
estado as "Estado Factura", f.fecha_factura as "Fecha Factura", f.idPedido as Pedido, 
d.idArticulo as Articulo,   a.nombre as Descripcion,  c.nombre as Clasificacion, d.cantidad as Cantidad, d.precio as Precio, p.simbolo_moneda as moneda,
sum( d.cantidad * d.precio ) as PrecioTotal
from factura f 
inner join detalle_factura  d on f.idNumeroFact = d.idNumeroFact
inner join articulos a on a.idArticulo = d.idArticulo 
inner join clasificacion c on c.idClasificacion = a.idClasificacion
inner join listaprecio p on p.idArticulo = a.idArticulo 
where f.estado = 'FACTURADO' and  year(f.fecha_factura ) = 2024
GROUP BY
estado , f.fecha_factura , f.idPedido , d.idArticulo,  a.nombre , d.cantidad, d.precio,c.nombre ,p.simbolo_moneda
order by f.idPedido ;


create view view_lista_precio_mayor as
select l.idlista as ListaPrecio ,rtrim(a.idArticulo) as Articulo ,a.nombre as Descripcion, l.precio as Precio, l.simbolo_moneda as Moneda
from articulos a
inner join listaprecio l on l.idArticulo = a.idArticulo 
where l.precio > 10 and l.cod_moneda = '2' ;

create view view_articulos_importados_x_proveedor as
select rtrim(a.idArticulo) as Articulo, rtrim(a.nombre) as Descripcion, p.idProveedor as Proveedor, p.nombre as Nombre
from articulos a
...
/*funcion 1*/
DELIMITER $$
CREATE FUNCTION get_articulo_proveedor (p_idProvedor varchar(10) )
RETURNS VARCHAR(255)
...
BEGIN
    ...
    SET resultado = (
    select concat("El Proveedor:" ,' ' , a.nombre, ' distribuye ' , count(b.idArticulo) , ' ' , 'articulos' )  as pedidos
	from proveedor a
	left join articulos b on a.idProveedor = b.idProveedor
	where a.idProveedor = p_idProvedor  
	group by   a.nombre
    order by count(b.idArticulo) desc
    );
    RETURN resultado;
END
$$

 /*select idProveedor, get_articulo_proveedor(idProveedor) as Resultado from proveedor*/

/*funcion 2*/
DELIMITER $$
CREATE FUNCTION get_factura_hasta_fechaActual (p_fecha datetime )
RETURNS VARCHAR(255)
...
BEGIN
	...
	from factura f
	where
	f.fecha_factura <= CURDATE()
    );
    RETURN resultado;
END
$$

/*select f.idNumeroFact as "Numero Factura", f.estado, f.idPedido as Pedido,get_factura_hasta_fechaActual(CURDATE() ) as "Fecha Factura"
from factura f ;*/


/*funcion 3*/
DELIMITER $$
CREATE FUNCTION get_precioPorMoneda (precio decimal(20,5), cod_moneda int )
RETURNS VARCHAR(255)
...
BEGIN   
    CASE 
        WHEN (precio BETWEEN 15.0 AND 20.0 ) AND cod_moneda = 2 THEN RETURN 'MONTO BAJO EN USD ESTA ENTRE 15.0 Y 20.0';
        WHEN (precio >= 21.0 AND  precio <= 40.0 AND cod_moneda = 2 )  THEN RETURN 'MONTO MEDIO EN USD ESTA ENTRE 21.0 Y 40.0';
        WHEN (precio >= 41.0  AND cod_moneda = 2 ) THEN RETURN 'MONTO ALTO EN USD ES IGUAL Ó MAYOR LOS 41.0';
        WHEN precio < 15.0  THEN RETURN 'MONTO BASICO EN USD ES MENOR A 15.0';
        
        WHEN (precio BETWEEN 10000.0 AND 50000.0) AND cod_moneda = 1 THEN RETURN 'MONTO BAJO EN PESOS ESTA ENTRE 10000.0 Y  50000.0';
        WHEN (precio >= 51000.0 AND  precio <= 71000.0  AND cod_moneda = 1 )  THEN RETURN 'MONTO MEDIO EN PESOS ESTA ENTRE 51000.0 Y 71000.0';
        WHEN (precio > 71000.0  AND cod_moneda = 1 ) THEN RETURN 'MONTO ALTO EN PESOS ES IGUAL Ó MAYOR  LOS 71000.0 ';
		WHEN precio  < 10000.0 THEN RETURN 'MONTO BASICO EN PESOS ES MENOR A 10000.0 ';
    END CASE;
END
$$

/*select *,get_precioPorMoneda ( precio, cod_moneda) as resultado from listaPrecio;*/
...
CREATE PROCEDURE sp_ordenar_tabla_campo(
    IN p_campo VARCHAR(255), 
    IN p_tipo_ordenamiento VARCHAR(255), 
    IN p_tabla VARCHAR(255)
)
BEGIN
    DECLARE ordenar VARCHAR(255);
    DECLARE tipo VARCHAR(255);
    DECLARE clausula VARCHAR(255);
    DECLARE error_message VARCHAR(255);
    DECLARE valida_tabla_campo INT;
    DECLARE valida_tabla INT;
    DECLARE valida_campo INT;

    -- Consulta para saber si campo y tabla ingresados existen en el esquema de la base de datos
    SELECT COUNT(*) INTO valida_tabla_campo  
    FROM information_schema.columns 
    WHERE table_schema = 'centro_diagnostico_investigacion' 
    AND table_name = p_tabla 
    AND column_name = p_campo;
    
    -- Consulta para saber si la tabla ingresados existen en el esquema de la base de datos
    SELECT COUNT(*) INTO valida_tabla  
    FROM information_schema.columns 
    WHERE table_schema = 'centro_diagnostico_investigacion' 
    AND table_name = p_tabla; 
   
    -- Consulta para saber si la tabla ingresados existen en el esquema de la base de datos
    SELECT COUNT(*) INTO valida_campo  
    FROM information_schema.columns 
    WHERE table_schema = 'centro_diagnostico_investigacion' 
	AND column_name = p_campo;
    ...
    IF p_campo IS NULL OR p_campo = ''  OR valida_campo = 0 THEN
        SET error_message = CONCAT('El campo ingresado no puede estar vacío, nulo o no existe, revise e ingrese nuevamente.');
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_message;
    END IF;
    
    -- Validar que la tabla no esté vacío
	IF p_tabla IS NULL OR p_tabla = ''  OR valida_tabla = 0 THEN
        SET error_message = CONCAT('La tabla ingresada no puede estar vacío, nulo o no existe, revise e ingrese nuevamente.');
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_message;
    END IF;
    
     -- Verificar si el tipo de ordenamiento es válido
	IF p_tipo_ordenamiento NOT IN ('ASC', 'DESC') THEN
		SET error_message = CONCAT(case when p_tipo_ordenamiento = '' THEN 'Ingreso vacio ' ELSE p_tipo_ordenamiento END, ' es un ordenamiento inválido. Ingrese ASC o DESC.');
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_message;
	END IF;
    
	-- Verificar si el campo y la tabla existen en la db y pertenecen a la misma tabla 
    IF valida_tabla_campo = 0 THEN 
        SET error_message = CONCAT('El campo ', p_campo, ' no existe o no pertenece a la tabla ', p_tabla);
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_message;

    ELSE
        IF p_campo <> '' OR p_campo IS NOT NULL THEN
            SET ordenar = CONCAT(' ORDER BY ', p_campo);
        END IF;

        SET tipo = CONCAT(' ', p_tipo_ordenamiento);
        
    END IF;

    SET @clausula = CONCAT('SELECT * FROM ', p_tabla, ordenar, tipo);
...
-- CALL sp_ordenar_tabla_campo('estado','asc','factura');
 
 /*store procedure 2*/
 
DELIMITER $$
CREATE PROCEDURE sp_estado_fact (IN p_estado varchar(20) )
BEGIN   
 DECLARE error_message VARCHAR(255);
 
	IF p_estado NOT IN ('FACTURADO','PENDIENTE')  THEN
        SET error_message = CONCAT('El valor ingresado no corresponde a un estado de la factura ingrese nuevamente.');
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = error_message;
    END IF;
	
    IF p_estado = 'FACTURADO' THEN
		select f.idNumeroFact as "Numero Factura", f.estado as 'Estado' ,f.fecha_factura as 'Fecha'
		from factura f
...
END $$
/* CALL sp_estado_fact('PENDIENTE')*/



/* trigger de alta*/
DELIMITER $$
CREATE TRIGGER trigger_proveedor_historico_alta
AFTER INSERT ON proveedor
FOR EACH ROW
BEGIN
    INSERT INTO proveedor_historico (idProveedor,nombre,razon_social,telefono,email,direccion,idPais,idCiudad,idProvincia,fecha_mod,usuario_mod,operacion) 
    VALUES (NEW.idProveedor, NEW.nombre, NEW.razon_social,NEW.telefono,NEW.email,NEW.direccion,NEW.idPais,NEW.idCiudad,NEW.idProvincia, curdate(),USER(),'ALTA');
END
$$

/*
INSERT INTO proveedor (idProveedor, nombre, razon_social, telefono, email, direccion, idPais, idCiudad, idProvincia) VALUES ('P0370', 'CAPSULANDIA', 'DAVID FERNANDO SELEM', '4546-9050', 'CAPSULANDIA@sa.com', 'ECHEVERRIA 2525', '032', '0000019527', '901');
INSERT INTO proveedor (idProveedor, nombre, razon_social, telefono, email, direccion, idPais, idCiudad, idProvincia) VALUES ('P0356', 'NOXIESTORE S.A.', 'NOXIESTORE S.A.', '11-24691760', 'NOXIESTORE@gmail.com', 'AV. ALVAREZ THOMAS 937 2º 203', '032', '0000019527', '901');
INSERT INTO proveedor (idProveedor, nombre, razon_social, telefono, email, direccion, idPais, idCiudad, idProvincia) VALUES ('P0353', 'FARMACIAS DR. AHORRO', 'ENERGIA Y VIDA DE ARGENTINA S.A.', '5555-9999', 'dr@gmail.com', 'AV. CORRIENTES 5283', '032', '0000019527', '901');
INSERT INTO proveedor (idProveedor, nombre, razon_social, telefono, email, direccion, idPais, idCiudad, idProvincia) VALUES ('P0344', 'ZG MULTITEC S.R.L.', 'ZG MULTITEC S.R.L.', '236-4639180', 'zg@gmail.com', 'QUINTANA 257', '032', '0000019527', '901');
*/

/*triger actualizacion*/
DELIMITER $$
CREATE TRIGGER trigger_producto_historico_update
AFTER UPDATE ON proveedor
FOR EACH ROW
BEGIN
	INSERT INTO proveedor_historico (idProveedor,nombre,razon_social,telefono,email,direccion,idPais,idCiudad,idProvincia,fecha_mod,usuario_mod,operacion) 
    VALUES (OLD.idProveedor, OLD.nombre, OLD.razon_social,OLD.telefono,OLD.email,OLD.direccion,OLD.idPais,OLD.idCiudad,OLD.idProvincia, curdate(),USER(),'MODIFICACION');
END
$$
...
UPDATE proveedor 
SET email = 'ZGMULTITEC@gmail.com', telefono = '4888-9999'
where idProveedor = 'P0353'

UPDATE proveedor 
SET email = 'farmaciadrahorro@gmail.com'
where idProveedor = 'P0353'
*/

/*trigger eliminacion*/
DELIMITER $$
CREATE TRIGGER trigger_producto_historico_delete
AFTER DELETE ON proveedor
FOR EACH ROW
BEGIN
	INSERT INTO proveedor_historico (idProveedor,nombre,razon_social,telefono,email,direccion,idPais,idCiudad,idProvincia,fecha_mod,usuario_mod,operacion) 
    VALUES (OLD.idProveedor, OLD.nombre, OLD.razon_social,OLD.telefono,OLD.email,OLD.direccion,OLD.idPais,OLD.idCiudad,OLD.idProvincia, curdate(),USER(),'ELIMINACION');
END
$$

/*DELETE FROM proveedor WHERE idProveedor in ( 'P0370'  ,'P0356')*/
